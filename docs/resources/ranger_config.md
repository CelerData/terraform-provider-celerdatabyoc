...
A Ranger configuration specifies the remote storage paths to the configuration files required to integrate a CelerData cluster to Apache Ranger, which provides a centralized security management framework that allows users to customize access policies through a visual web page. You can apply the configuration when creating or modifying the cluster using [celerdatabyoc_elastic_cluster_v2](./elastic_cluster_v2.md).
